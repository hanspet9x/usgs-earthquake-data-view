...
const MainContainer = () => {
  ...
  const { data } = useSetEarthquakeData();
        ...
        // const getScreenData = () => {
            switch (type) {
              ...
              case "Location":
               ...
               case "Status":
                return Object.entries(data.sectioned.status).map(
                    ([nKey, value], i) => (
                      <MainContentTitle key={i} title={nKey} data={value} />
                    )
                  );
    ...
    }, [data?.all.features, data.sectioned.earthquake, data.sectioned.location, data.sectioned.quarry, data.sectioned.status, data.sectioned.tsunami, type])
